...
fn lfw_init_and_add_create_rust_workflow_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(init["created"].as_array().unwrap().iter().any(|path| {
        path.as_str()
            .unwrap()
            .ends_with("workflows/example/src/lib.rs")
    }));
    assert!(init["created"].as_array().unwrap().iter().any(|path| {
        path.as_str()
            .unwrap()
            .ends_with("workflows/example/.agent/skills/lightflow-example/SKILL.md")
    }));

    let old_category_flag = Command::new(env!("CARGO_BIN_EXE_lfw"))
        .args(["new", "old_category", "--category", "examples"])
        .current_dir(&root)
        .output()?;
    assert!(!old_category_flag.status.success());
    assert!(String::from_utf8_lossy(&old_category_flag.stderr).contains("usage:"));

    let added = lfw(&root, ["new", "extra", "--name", "Extra Workflow"])?;
    assert_eq!(added["workflow_id"], "lightflow.extra");
    assert!(added.get("category").is_none());
    let manifest = fs::read_to_string(root.join(".lightflow/workflows/extra/Cargo.toml"))?;
    ...
    assert!(!root.join(".lightflow/workflows/extra/src/main.rs").exists());
    complete_generated_workflow_metadata(&root, "example")?;
    complete_generated_workflow_metadata(&root, "extra")?;
...
fn lfw_new_and_add_support_global_workflow_workspace() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let global = lfw(&root, ["new", "-g", "global_tool", "--name", "Global Tool"])?;
    ...
    assert!(global_root.join("global_tool/src/lib.rs").exists());
    assert!(!root.join("workflows/global_tool/src/lib.rs").exists());
...
fn lfw_new_runtime_template_creates_node_contract_files() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let crate_dir = root.join(".lightflow/workflows/my_flux_sampler");
