...
pub fn mcp_response(service: &ApiService, request: Value) -> Value {
    mcp::handle_request(service, request)
}

pub fn mcp_resource_text(result: &Value) -> &str {
    result["contents"][0]["text"]
        .as_str()
        .expect("MCP resource text")
}

pub fn mcp_resource_json(result: &Value) -> serde_json::Result<Value> {
    serde_json::from_str(mcp_resource_text(result))
}

pub fn assert_mcp_error(response: &Value, code: i64, message: &str) {
